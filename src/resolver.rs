...
/// parser/kotlin.rs — but never a value-receiver, which the parser strips
...
fn resolve_single_call(
    ...
    let (last, imports_hint): (&str, Vec<String>) =
        if callee.contains("::") || callee.contains(provider.import_separator()) {
            (provider.import_last_segment(callee), vec![callee.to_string()])
        } else {
            (callee, file_imports.get(file_id).cloned().unwrap_or_default())
        };
    let candidates = match idx.by_name.get(last) {
        Some(c) => c,
        None => return PolicyResolution::NotFound,
    };
    let ev = crate::call_evidence::CallEvidence { imports_hint: &imports_hint, caller_file: file_id };
